...
<body>
    <div class="page">
            ...
            <tr>
                <td style="border: none; padding: 3px 5px; width: 50%;">
                    <div class="info-box">
                        ...
                        <div class="info-value">{{ $order->patient->full_name ?? 'N/A' }}</div>
                    </div>
                </td>
                <td style="border: none; padding: 3px 5px; width: 50%;">
                    <div class="info-box">
                        <div class="info-label">Order Number</div>
                        <div class="info-value">#{{ $order->lab_test_order_id }}</div>
                    </div>
                </td>
            </tr>
            <tr>
                <td style="border: none; padding: 3px 5px;">
                    <div class="info-box">
                        ...
                        <div class="info-value">{{ $order->physician->physician_name ?? 'Not specified' }}</div>
                    </div>
                </td>
                <td style="border: none; padding: 3px 5px;">
                    <div class="info-box">
                        <div class="info-label">Order Date</div>
                        <div class="info-value">{{ $order->order_date ? $order->order_date->format('F d, Y - h:i A') : 'N/A' }}</div>
                    </div>
                </td>
            </tr>
            <tr>
                    ...
                    <div class="info-box">
                        ...
                        <div class="info-value">
                            ...
                            @if($order->patient->birthdate)
                                / {{ $order->patient->birthdate->format('M d, Y') }}
                                ({{ $order->patient->birthdate->age }} yrs)
                            @endif
                        </div>
                    </div>
                </td>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($flag)
                            <span class="flag {{ $flagClass }}">{{ $flag }}</span>
                        @else
                            —
                        @endif
                    </td>
                    <td>{{ $result->remarks ?? ($result->findings ?? '—') }}</td>
                    <td>
                        @if($result)
        ...
        <div class="remarks-box">
            ...
            <div class="text">{{ $order->remarks }}</div>
        </div>
        @endif
                            ...
                            <div class="sig-name">
                                @php
                                    $performers = $order->orderTests
                                        ...
                                        ->unique()->implode(', ');
                                @endphp
                                {{ $performers ?: '________________________' }}
                            </div>
                            ...
                            <div class="sig-name">
                                @php
                                    $verifiers = $order->orderTests
                                        ...
                                        ->unique()->implode(', ');
                                @endphp
                                {{ $verifiers ?: '________________________' }}
                            </div>
