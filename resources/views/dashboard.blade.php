...
// Optimize queries with a single DB call using caching
$stats = Cache::remember('dashboard_stats', 300, function() {
    return [
        'patients' => Patient::where('is_deleted', 0)->count(),
        'patients_today' => Patient::where('is_deleted', 0)->whereDate('datetime_added', today())->count(),
        'items' => Item::where('is_deleted', 0)->count(),
        'equipment' => Equipment::where('is_deleted', 0)->count(),
        'transactions' => Transaction::count(),
    ];
});

// Cache recent activities for 5 minutes
$recentActivities = Cache::remember('recent_activities', 300, function() {
    return ActivityLog::with(['employee' => function($query) {
        $query->select('employee_id', 'firstname', 'lastname', 'section_id');
    }])->orderBy('datetime_added', 'desc')->limit(10)->get();
});

// Extract cached values
$patientsCount = $stats['patients'];
$patientsTodayCount = $stats['patients_today'];
$itemsCount = $stats['items'];
$equipmentCount = $stats['equipment'];
