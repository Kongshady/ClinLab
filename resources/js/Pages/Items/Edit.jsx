...
export default function Edit({ item, sections }) {
    const { data, setData, put, processing } = useForm({ section_id: item.section_id, label: item.label, unit: item.unit, reorder_level: item.reorder_level });
    return (
        <AppLayout title="Edit Item">
            <div className="bg-white shadow-sm sm:rounded-lg p-6">
                <form onSubmit={(e) => { e.preventDefault(); put(`/items/${item.item_id}`); }} className="space-y-4 max-w-2xl">
