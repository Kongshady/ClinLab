...
export default function Items({ items, sections, flash }) {
    ...
    const handleDelete = (id) => {
        if (confirm('Are you sure you want to delete this item?')) {
            router.delete(`/items/${id}`, {
                onSuccess: () => setFlashMessage('Item deleted successfully.')
            });
        }
    };
    ...
    const filteredItems = items.data.filter(i => 
        ...
        (i.section?.label && i.section.label.toLowerCase().includes(searchTerm.toLowerCase()))
    );
        ...
        <AppLayout title="Inventory Management">
            <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div className="p-8">
                    ...
                    {/* Items List */}
                    <div>
                        <h3 className="text-lg font-semibold text-gray-800 mb-4">Items List</h3>
                            ...
                            <table className="min-w-full">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Reorder Level</th>
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {filteredItems.length > 0 ? filteredItems.map((item) => (
                                        <tr key={item.item_id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 text-sm text-gray-600">{item.section?.label || 'N/A'}</td>
                                            <td className="px-6 py-4 text-sm text-gray-600">{item.unit || 'pcs'}</td>
                                            <td className="px-6 py-4 text-sm text-gray-600">{item.reorder_level || '-'}</td>
                                            <td className="px-6 py-4 text-sm space-x-2">
                                                <button
                                                    onClick={() => router.get(`/items/${item.item_id}/edit`)}
